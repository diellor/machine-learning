...
def main(args: argparse.Namespace) -> float:
    ...
    ones = np.ones([442])
    inputs_with_bias = np.column_stack((dataset.data, ones))
    print(inputs_with_bias)
    # TODO: Split the dataset into a train set and a test set.
    # Use `sklearn.model_selection.train_test_split` method call, passing
    # arguments `test_size=args.test_size, random_state=args.seed`.
    X_train, X_test, Y_train, Y_test = sklearn.model_selection.train_test_split(inputs_with_bias,dataset.target,test_size=args.test_size,random_state=args.seed)

    # TODO: Solve the linear regression using the algorithm from the lecture,
    # explicitly computing the matrix inverse (using `np.linalg.inv`).
    weights = np.linalg.inv(X_train.T.dot(X_train)).dot(X_train.T).dot(Y_train)
    # TODO: Predict target values on the test set.
    targetVals = X_test @ weights

    # TODO: Manually compute root mean square error on the test set predictions.
    rmse = np.sqrt(np.square(np.subtract(Y_test,targetVals)).mean())
