...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    train_data, test_data, train_target, test_target \
        = sklearn.model_selection.train_test_split(data, target, test_size=args.test_size,
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, train_data.shape[0], args.batch_size):
            ...
            gradients = np.dot(xi.T, sigmoid(np.dot(xi, weights.T)) - yi) / yi.shape[0]
            weights = weights - args.learning_rate * gradients.T

            prediction = sigmoid(np.dot(train_data, weights.T))
            train_l.append(loss(prediction, train_target))

            prediction_modified =[]
            for i in prediction:
                if i > 0.5:
                    prediction_modified.append(1)
                else:
                    prediction_modified.append(0)

            train_acc.append(np.mean(prediction_modified == train_target))

            prediction = sigmoid(np.dot(test_data, weights.T))
            test_l.append(loss(prediction, test_target))
            ...
            test_acc.append(np.mean(prediction_modified == test_target))
        ...
        train_accuracy = train_acc[len(train_acc)-1]
        train_loss = train_l[len(train_l)-1]
        test_accuracy = test_acc[len(test_acc) -1]
        test_loss = test_l[len(test_l)-1]
